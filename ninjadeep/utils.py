...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import ninjadeep.utils
        import importlib
        path = Path(f"ninjadeep/plugins/{shortname}.py")
        name = "ninjadeep.plugins.{}".format(shortname)
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = ninjadeep.utils
        mod.Config = Config
        mod.borg = bot
        mod.ninjadeep = bot
        # auto-load
        ...
        sys.modules["ninjadeep.events"] = ninjadeep.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["ninjadeep.plugins." + shortname] = mod
        print("Successfully (re)imported " + shortname)
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"ninjadeep.plugins.{shortname}"
...
def start_mybot(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ninjadeep/plugins/mybot/{shortname}.py")
        name = "ninjadeep.plugins.mybot.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["ninjadeep.plugins.mybot" + shortname] = mod
