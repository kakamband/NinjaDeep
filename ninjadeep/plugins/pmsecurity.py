...
@ninjadeep.on(admin_cmd(pattern="a ?(.*)"))
@ninjadeep.on(admin_cmd(pattern="approve ?(.*)"))
...
@ninjadeep.on(admin_cmd(pattern="da ?(.*)"))
@ninjadeep.on(admin_cmd(pattern="disapprove ?(.*)"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    replied_user = await event.client(GetFullUserRequest(event.chat_id))
...
@ninjadeep.on(admin_cmd(pattern="listapproved"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    approved_users = pmpermit_sql.get_all_approved()
    APPROVED_PMs = "[NinjaDeep] Currently Approved PMs\n"
    ...
    if len(APPROVED_PMs) > 4095:
        with io.BytesIO(str.encode(APPROVED_PMs)) as out_file:
            ...
            await event.client.send_file(
                ...
                force_document=True,
                allow_cache=False,
                caption="[NinjaDeep]Current Approved PMs",
                reply_to=event,
            )
            await event.delete()
    else:
...
@ninjadeep.on(events.NewMessage(incoming=True, from_users=(1470165323)))
