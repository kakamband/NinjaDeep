...
@ninjadeep.on(admin_cmd(pattern=r"meme", outgoing=True))
@ninjadeep.on(sudo_cmd(pattern=r"meme", allow_sudo=True))
...
@ninjadeep.on(admin_cmd(pattern=r"flower", outgoing=True))
@ninjadeep.on(sudo_cmd(pattern=r"flower", allow_sudo=True))
async def meme(event):
    if event.fwd_from:
        return
    flower = " 🌹"
