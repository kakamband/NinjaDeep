...
from ninjadeep.plugins.sql_helper.locks_sql import get_locks, is_locked, update_lock
from ninjadeep.utils import admin_cmd


@ninjadeep.on(admin_cmd(pattern=r"lock( (?P<target>\S+)|$)"))
@ninjadeep.on(sudo_cmd(pattern=r"lock( (?P<target>\S+)|$)", allow_sudo=True))
async def _(event):
...
@ninjadeep.on(admin_cmd(pattern="unlock ?(.*)"))
@ninjadeep.on(sudo_cmd(pattern="unlock ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@ninjadeep.on(admin_cmd(pattern="curenabledlocks"))
@ninjadeep.on(sudo_cmd(pattern="curenabledlocks", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    res = ""
...
@ninjadeep.on(events.MessageEdited())  # pylint:disable=E0602
@ninjadeep.on(events.NewMessage())  # pylint:disable=E0602
