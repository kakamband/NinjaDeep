...
# Rewritten for NinjaDeep by @xditya

"""
This module is used for updating NinjaDeep
"""
...
GIT_REPO_NAME = "NinjaDeep"
UPSTREAM_REPO_URL = "https://github.com/Lovedeep-ViRk/NinjaDeep"
...
@ninjadeep.on(admin_cmd(pattern="uNinja ?(.*)"))
async def upstream(ups):
    "For .update command, check if the bot is up to date, update if specified"
    await ups.edit("`Searching for new updates, if any For NinjaDeep...`")
    ...
    try:
        ...
        if conf != "now":
            ...
            \nBut we can fix that by force updating the userbot using** `{CMD_HNDLR}uNinja now`."
            )
            return
        repo = Repo.init()
    ...
    if ac_br != "master":
        await ups.edit(
            ...
            "Please checkout the official branch of NinjaDeep`"
        )
        repo.__del__()
        return
    ...
    if conf != "now" and not force_updateme:
        ...
        if len(changelog_str) > 4096:
            ...
            remove("NinjaDeep.txt")
        else:
            await ups.edit(changelog_str)
        await ups.respond(f"Do `{CMD_HNDLR}uNinja now` to update NinJaDeep")
        return
    ...
    if Var.HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await ups.edit(
            "`NinjaDeep dyno build in progress, please wait for it to complete.`"
        )
        ups_rem.fetch(ac_br)
        ...
        await ups.edit(
            "`Successfully Updated!\n" "NinjaDeep is restarting... Wait for a second!`"
        )
