...
from ninjadeep.plugins.mybot import started, helpmefast, forping
import html
...
DEF = str(ALIVE_NAME) if ALIVE_NAME else "NinjaDeep"
...
async def thisfn(event):
    await tgbot.send_message(
        ...
        message=started,
        buttons=[
            [Button.url("Repository", "https://github.com/Lovedeep-ViRk/NinjaDeep/")]
        ]
    )
...
async def thisfn(event):
    await tgbot.send_message(
        ...
        link_preview=False,
        buttons=[
            [Button.url("NinjaDeep", "https://t.me/NinjaDeepSUPPORT")]
        ]
    )
...
async def thisfn(event):
    await tgbot.send_message(
        ...
        message=forping,
        buttons=[
            [Button.url("Deploy", "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2FLovedeep-ViRk%2FNinjaDeep&template=https%3A%2F%2Fgithub.com%2FLovedeep-ViRk%2FNinjaDeep")]
        ]
    )
...
async def _(event):
    ...
    try:
        ...
        str(e)
    caption = """Extracted Userdata From ninjadeep's DATABASE
ID: <code>{}</code>
...
""".format(
    ...
    await tgbot.send_message(
        ...
        silent=True,
        buttons=[
            [Button.url("More", "https://t.me/NinjaDeepOT")]
        ]
    )
...
async def _(givelogs):
    ...
    if givelogs.sender_id == bot.uid:
        await tgbot.send_file(
            ...
            buttons=[
                [Button.url("Crashed?", "t.me/NinjaDeepOT")]
            ])
    else:
        resp = f"This option is available only for my master, {DEF}!"
