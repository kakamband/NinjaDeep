...
@ninjadeep.on(admin_cmd(pattern="sudo"))
async def sudo(event):
    sudo = "True" if Config.SUDO_USERS else "False"
    users = os.environ.get("SUDO_USERS", None)
    if sudo == "True":
        await eor(event, f"**ninjadeep**\nSudo - `Enabled`\nSudo user(s) - `{users}`")
    else:
        await eor(event, f"**ninjadeep**\nSudo - `Disabled`")


@ninjadeep.on(admin_cmd(pattern="prefix"))
...
@ninjadeep.on(admin_cmd(pattern="addsudo(?: |$)"))
async def tb(event):
    ok = await eor(event, "Adding user as a sudo...")
    ninjadeep = "SUDO_USERS"
