...
@ninjadeep.on(admin_cmd(pattern="botgban ?(.*)"))
@ninjadeep.on(sudo_cmd(pattern="botgban ?(.*)", allow_sudo=True))
async def _(event):
...
@ninjadeep.on(admin_cmd(pattern="botungban ?(.*)"))
@ninjadeep.on(sudo_cmd(pattern="botungban ?(.*)", allow_sudo=True))
async def _(event):
    if Config.G_BAN_LOGGER_GROUP is None:
        await eor(
            event,
